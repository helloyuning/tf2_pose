...
def DenseNet(img_input=None, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1, nb_layers_per_block=-1,
...
def __transition_up_block(ip, nb_filters, type='deconv', weight_decay=1E-4):
    ...
    if type == 'upsampling':
        ...
        x = pixel.SubPixelUpscaling(scale_factor=2)(x)
        x = Conv2D(nb_filters, (3, 3), activation='relu', padding='same', kernel_regularizer=l2(weight_decay),
...
def __create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
    ...
    if subsample_initial_block:
        ...
        initial_kernel = (3, 3)
        initial_strides = (1, 1)
    img_input = tf.concat([img_input[2], img_input[3]], axis=0)
    ...
    x = tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=7, padding='same')(x)
    x = Activation('relu')(x)
    x = tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=8, padding='same')(x)
    x = Activation('relu')(x)

    np_matrix = np.identity(4)
    identity_q = matrix2quaternion(np_matrix)
    weights_r = tf.Variable(initial_value=tf.random.normal([1, 1, 64, 4], stddev=0.001), shape=[1, 1, 64, 4],
                            dtype=tf.float32, name='weights_r')
    biases_r = tf.Variable(initial_value=identity_q, shape=[4], dtype=tf.float32, name='biases_r')
    conv = tf.nn.conv2d(x, weights_r, strides=[1, 1, 1, 1], padding='VALID')  # 1x1x4
    ...
    weights_t = tf.Variable(initial_value=tf.random.normal([1, 1, 64, 3], stddev=0.001), shape=[1, 1, 64, 3],
                            dtype=tf.float32,
                            name='weights_t')
    ...
    conv = tf.nn.conv2d(x, weights_t, strides=[1, 1, 1, 1], padding='VALID')  # 1x1x3
