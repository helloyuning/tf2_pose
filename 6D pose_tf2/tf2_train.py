...
def my_gen(img_list, batch_size, point):
...
def get_total_data():
    ...
    for obj in objects[0]:
        ...
        for frame in bench.frames:
            ...
            # hypo_rot = img_to_array(hypo_rot.reshape(1, 4))
            # hypo_rot = np.expand_dims(hypo_rot, axis=0)
            # hypo_trans = img_to_array(hypo_trans.reshape(1, 3))
            # hypo_trans = np.expand_dims(hypo_trans, axis=0)
            # crop_shift = img_to_array(crop_shift)
            ...
            # data_collection.append([[scene_patch], [render_patch]])
            contour_collection.append(contour_gt)
            #gtpose_collection.append(gt_pose)
...
def train_step(model, data, point, optimizer, num):
    ...
    with tf.GradientTape() as tape:
        ...
        # data1 = tf.concat([data[0], data[1]], axis=0)
...
if __name__ == '__main__':
    ...
    for num in range(len(data)):
        #train(model, data[0], point[0], opt)
